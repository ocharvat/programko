while (true)
{
    Console.WriteLine("Test teorie autoškola");
    Console.WriteLine();
    Console.WriteLine("Vybírej z možností A,B nebo A,B,C");
    Console.WriteLine("Po ukončení se zobrazí výsledek (nejde ale posoudit, jestli jsi prospěl, protože v realitě bývají otázky různě bodově ohodnoceny)");
    ...
    do
    {
        ...
        Console.Clear();
        Console.WriteLine();
        int.TryParse(x, out pocet);
        z = (int.TryParse(x, out pocet));
        if (!z)
        {
    ...
    for (int i = 0; i < pocet; i++)
    {
        ...
        Console.WriteLine(i + 1 + "/" + pocet);
        string vyberotazky()
        {
            string[] otazky = File.ReadAllLines("questions.data");
            Random rnd = new Random();
            int otazka = rnd.Next(0, 5);
    ...
    Console.WriteLine("počet správných odpovědí: " + successrate + "/" + pocet);
    Console.WriteLine("vase uspesnost je:" + (100 * successrate / pocet) + "%");
